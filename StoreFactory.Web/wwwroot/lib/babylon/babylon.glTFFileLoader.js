...
(function (BABYLON) {
    ...
    */
    ...
    var getAttribute = function (attributeParameter) {
        if (!attributeParameter || !attributeParameter.semantic) {
            return null;
        }
    ...
    */
    var onBindShaderMaterial = function (mesh, gltfRuntime, unTreatedUniforms, shaderMaterial, pass, material) {
        for (var unif in unTreatedUniforms) {
            ...
            if (type === EParameterType.FLOAT_MAT2 || type === EParameterType.FLOAT_MAT3 || type === EParameterType.FLOAT_MAT4) {
                ...
                if (!pass || !pass.instanceProgram) {
                    continue;
                }
                var materialValues = material.instanceTechnique ? material.instanceTechnique.values : material.values;
                var value = materialValues[pass.instanceProgram.uniforms[unif]];
                if (!value) {
                    continue;
                }
    ...
    var onShadersLoaded = function (gltfRuntime) {
        ...
        for (var mat in gltfRuntime.materials) {
            ...
            // Some exporters (including obj2gltf) emit glTF 1.0 materials using the
            // `technique` + `values` properties instead of `instanceTechnique`.
            // The original BabylonJS v2.2 loader expected `instanceTechnique`.
            ...
            if (!instanceTechnique || !instanceTechnique.technique) {
                BABYLON.Tools.Warn("Material " + material.name + " is missing a technique; skipping.");
                continue;
            }
            var technique = gltfRuntime.techniques[instanceTechnique.technique];
            // Skip shader material creation to avoid WebGL uniform errors from legacy GLSL pipelines.
            // The meshes will be assigned materials in the viewer instead.
            if (!technique) {
                BABYLON.Tools.Warn("Technique missing for material " + material.name + "; skipping shader material.");
                continue;
            }
            if (gltfRuntime.scene && gltfRuntime.scene.defaultMaterial) {
                var fallback = gltfRuntime.scene.defaultMaterial;
                fallback.id = mat;
            }
            else {
                BABYLON.Tools.Warn("No scene default material available; skipping shader material for " + material.name);
            }
        }
        // Finish
...
// (source map removed to avoid 404 in dev console)